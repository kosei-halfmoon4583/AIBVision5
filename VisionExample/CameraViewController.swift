  ...
  private func scanBarcodesOnDevice(in image: VisionImage, width: CGFloat, height: CGFloat) {
    ...
    DispatchQueue.main.sync {
      ...
      if let scanningError = scanningError {
        print("Error: バーコードスキャンNG! \(scanningError.localizedDescription).")
        return
      }
...
/*      理想的な検出設定を決定するためにレイテンシ テストを実行する場合は、正確なパフォーマンス メトリックを取得するために、アプリを「リリース」モードで実行します。*/
    ...
    DispatchQueue.main.sync {
      ...
      if let detectionError = detectionError {
        print("Error: 顔認識 NG! \(detectionError.localizedDescription).")
        return
      }
      guard !faces.isEmpty else {
        print("On Device 顔認識 検出結果なし！")
        return
      }
  ...
  private func detectPose(in image: MLImage, width: CGFloat, height: CGFloat) {
    if let poseDetector = self.poseDetector {
      ...
      DispatchQueue.main.sync {
        ...
        strongSelf.updatePreviewOverlayViewWithLastFrame()
        if let detectionError = detectionError {
          print("Error: 姿勢認識 NG! \(detectionError.localizedDescription).")
          return
        }
  ...
  private func detectSegmentationMask(in image: VisionImage, sampleBuffer: CMSampleBuffer) {
    ...
    DispatchQueue.main.sync {
      ...
      if let segmentationError = segmentationError {
        print("Error: セグメンテーション実行 NG! \(segmentationError.localizedDescription).")
        return
      }
      guard let mask = mask else {
        print("セグメンターが空のマスクを返しました！")
        return
      }
      guard let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) else {
        print("サンプル バッファーからイメージ バッファーを取得できませんでした！")
        return
      }
  ...
  private func recognizeTextOnDevice(
    ...
    DispatchQueue.main.sync {
      ...
      strongSelf.updatePreviewOverlayViewWithLastFrame()
      if let detectionError = detectionError {
        print("Error: 文字認識 NG! \(detectionError.localizedDescription).")
        return
      }
  ...
  private func detectLabels(
    ...
    if shouldUseCustomModel {
      guard
        ...
        print("On Device カスタム モデルが見つからなかったため、ラベル検出に失敗しました！")
        return
      }
    ...
    DispatchQueue.main.sync {
      ...
      if let labelingError = labelingError {
        print("Error: 画像ラベリング NG! \(labelingError.localizedDescription)")
        return
      }
  ...
  private func detectObjectsOnDevice(
    ...
    DispatchQueue.main.sync {
      ...
      if let detectionError = detectionError {
        print("Error: 物体検出 NG! \(detectionError.localizedDescription).")
        return

      }
      guard !objects.isEmpty else {
        print("On Device 物体検出結果なし！")
        return
      }
  ...
  private func setUpCaptureSessionInput() {
    ...
    sessionQueue.async {
      ...
      guard let device = strongSelf.captureDevice(forPosition: cameraPosition) else {
        print("カメラポジションの取得に失敗しました！: \(cameraPosition)")
        return
      }
      do {
        strongSelf.captureSession.beginConfiguration()
        ...
        guard strongSelf.captureSession.canAddInput(input) else {
          print("セッションイン取得に失敗しました！")
          return
        }
        strongSelf.captureSession.addInput(input)
        strongSelf.captureSession.commitConfiguration()
      } catch {
        print("Capture device inputの生成に失敗しました！: \(error.localizedDescription)")
      }
    }
  }
...
// claude.ai（推奨コード)
    private func presentDetectorsAlertController() {
        ...
        detectors.forEach { detectorType in
            let isEnabled = detectorType.rawValue != self.currentDetector.rawValue
            
            alertController.addAction(
                title: detectorType.rawValue,
                style: .default,
                isEnabled: isEnabled
            ) { [weak weakSelf] in
                guard let strongSelf = weakSelf else {
                ...
                guard let detector = Detector(rawValue: detectorType.rawValue) else { return }
        ...
        // Cancelアクションを追加
        alertController.addAction(
            title: Constant.cancelActionTitleText,
            style: .cancel,
            isEnabled: true,
            handler: nil
        )
...
extension CameraViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  func captureOutput(
    ...
    guard let inputImage = MLImage(sampleBuffer: sampleBuffer) else {
      print("サンプル バッファーから MLImage を生成できませんでした！")
      return
    }
